...
public class RunServiceImpl implements RunService {
    ...
    public RunServiceImpl(RunRepository runRepository, RunKafkaProducerServiceImpl runKafkaProducerService, @Qualifier("restApi2Client") RestClient restClient) {
        this.runRepository = runRepository;
        this.runKafkaProducerService = runKafkaProducerService;
    ...
    private void userExists(Integer userId) {
        try {
            ResponseEntity<String> result =
                    this.restClient.get().uri("/" + userId).retrieve().toEntity(String.class);
